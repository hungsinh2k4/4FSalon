...
const Appointment = () => {
	...
	useEffect(() => {
		...
		const loadAppointmentList = async () => {
			try {
				const data = await getAppointmentList(`user_id=${user?.id}&have_feedback=true&order=desc`);
	...
	const showError = (error: any) => {
		return (
			<div className="flex flex-col gap-3 w-full h-fit items-center">
				...
				<button className="w-fit h-fit px-2 py-1 bg-gray-300 hover:bg-gray-200"
			...
			<div className="relative bg-white px-4 pt-8 pb-6 shadow-xl ring-1 ring-gray-900/5 sm:ml-0 w-full max-w-4xl mx-10 sm:rounded-lg sm:px-10 overflow-auto overscroll-auto">	
				<div>
					...
					<div className="flex flex-row justify-start align-middle border-b-2 mb-3">
						<div className="font-bold hover:underline cursor-pointer select-none" 
					...
					<div className="relative min-h-full">
						...
						{appointmentList.slice((page-1)*pageSlice, (page-1)*pageSlice + pageSlice).map((appointment) => (
							<div key={appointment.id} className={`border border-gray-200 rounded-lg p-4 shadow-sm bg-white`}>
								<div className="flex justify-between items-center border-b pb-4 mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Lịch hẹn #{appointment.id}</h2>
                    <span className={`text-sm ${handleStatusColor(appointment.status)} px-3 py-1 rounded-full`}>{handleStatusDisplay(appointment?.status)}</span>
                </div> 
								<p className="text-gray-600">Dịch vụ: <span className="font-semibold">{appointment.service.title}</span></p>
								<p className="text-gray-600">Ngày hẹn: <span className="font-semibold">{beautifyDate(appointment.date)}</span></p>
								<p className="text-gray-600">Bắt đầu: <span className="font-semibold">{beautifyTime(appointment.start_time)}</span></p>
								<p className="text-gray-600">Địa chỉ: <span className="font-semibold">{appointment.branch.address}</span></p>
								<p className="text-gray-600">Thanh toán: <span className="font-semibold">{beautifyPrice(appointment.final_price)}</span></p>
								<div className="flex justify-end">
									<button className="text-white bg-blue-500 hover:bg-blue-600 focus:ring-4 focus:ring-blue-300 rounded px-4 py-2 text-sm font-medium w-fit h-fit"
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center border-b pb-4 mb-6">
                        <h2 className="text-2xl font-semibold text-gray-800">Lịch hẹn #{selectedAppointment.id}</h2>
                        <span className={`text-sm ${handleStatusColor(selectedAppointment.status)} px-3 py-1 rounded-full`}>{handleStatusDisplay(selectedAppointment?.status)}</span>
                    </div>
                      ...
                      <div className="space-y-2">
                          <h3 className="text-lg font-semibold text-gray-800">Thông tin lịch hẹn</h3>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Dịch vụ:</span>
                              <span className="text-gray-800 font-semibold">{selectedAppointment?.service.title}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Ngày hẹn:</span>
                              <span className="text-gray-800 font-semibold">{beautifyDate(selectedAppointment.date)}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Thời gian:</span>
                              <span className="text-gray-800 font-semibold">{beautifyTime(selectedAppointment.start_time)}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Ước tính kết thúc:</span>
                              <span className="text-gray-800 font-semibold">{beautifyTime(selectedAppointment.estimated_end_time)}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Thanh toán:</span>
                              <span className="text-gray-800 font-semibold">{beautifyPrice(selectedAppointment.final_price)}</span>
                          </div>
                      </div>
                      <div className="space-y-2">
                          <h3 className="text-lg font-semibold text-gray-800">Thông tin nhân viên</h3>
                          ...
                          <div className="flex justify-between">
                              ...
                              <span className="text-gray-800 font-semibold">{selectedAppointment.employee?.name}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">Email:</span>
                              <span className="text-gray-800 font-semibold">{selectedAppointment.employee?.email}</span>
                          </div>
                          <div className="flex justify-between">
                              <span className="text-gray-600 font-medium">SĐT:</span>
                              <span className="text-gray-800 font-semibold">{selectedAppointment.employee?.phone}</span>
                          </div>
                      </div>
										</div>
                    ...
                    <div className="container py-6">
                      <h3 className="text-lg font-semibold text-gray-800">Thông tin chi nhánh</h3>
                      <div className="flex items-center space-x-4">
                        ...
                        onClick={handleAppointmentListHeader}>
											...
											<button className={`text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:ring-red-300 rounded px-4 py-2 text-sm font-medium w-fit h-fit
											${
											...
											<button className={`text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:ring-green-300 rounded px-4 py-2 text-sm font-medium w-fit h-fit
											${
												selectedAppointment.status !== 'completed' ? 'hidden' : 'block'
												}`}
