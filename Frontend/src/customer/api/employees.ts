...
export const getEmployeeByBranchId = async (id: number | undefined): Promise<Employee> => {
  const response = await axiosInstance.get(`api/employees/search?branch_id=${id}`);
  return response.data;
};
...
export const getEmployeeDate = async (id: number, date: Date):Promise<Employee_date>  => {
  const response = await axiosInstance.get(`api/employees/${id}/avaiable/${date}`);
  return response.data;
}
